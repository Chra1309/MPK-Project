...
//// mapt die Farben des cubes fÃ¼r den solver auf die Farben des tatsÃ¤chlichen WÃ¼fel fÃ¼r die Ausgabe
void ClientCube::mapForCustomColor()
{	
	for(int j = 0; j < 6; j++){    
		for(int k = 0; k < 3; k++){
			for(int l = 0; l <3; l++){
				cube_customcolor[j][k][l] = lookup[data[j][k][l]];
			}
		}
	}
}
